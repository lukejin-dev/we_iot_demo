...
function initAllCharts(){    
    for (var chart in realtimeCharts){
        for(var idx=0;idx<realtimeCharts[chart].series.length;idx++)
...
function tiggerItemEvent(clientid){
    $("#monitor-item-"+clientid).addClass("monitor-item-div-active");
...
function okButtonHander(){   
    if(currentStatus=="Ready"){
        ...
        for (var idx in currentClientIDs){
            $("#monitor-div").append("<a href='javascript:void(0)'><div class='monitor-item-div' id='monitor-item-"+currentClientIDs[idx]+"'><span>"+currentClientIDs[idx]+"</span></div></a>");
        }
...
function realtimeThread(){    
    $.get(
    AJAX_BASE_URL+"action=read&time_after="+lastUpdateTime+"&limit=20",
    function(data) {
    ...
    function(data) {
       ...
       // console.log(data);
        ...
        for(var idx=0;idx<data.length;idx++){
            if(!data[idx]["time"]||Math.abs(now-parseInt(data[idx]["time"]))>60)
                continue;
            isEmpty=false;
        ...
        if(isEmpty) 
            $("#clients-select-div").append("<div class='checkbox'><label> - No Device Connected. Try Refresh the Page.</label></div>")
