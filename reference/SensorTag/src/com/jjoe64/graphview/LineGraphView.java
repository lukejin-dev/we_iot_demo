...
public class LineGraphView extends GraphView {
	...
	@Override
	public void drawSeries(Canvas canvas, GraphViewDataAdapterInterface values, float graphwidth, float graphheight, float border, double minX, double minY, double diffX, double diffY, float horstart, GraphViewSeriesStyle style) {
		// draw background
		double lastEndY = 0;
		double lastEndX = 0;
		if (drawBackground) {
			...
			for (int i = 0; i < values.size(); i++) {
				double valY = values.getY(i) - minY;
				double ratY = valY / diffY;
				double y = graphheight * ratY;

				double valX = values.getX(i) - minX;
