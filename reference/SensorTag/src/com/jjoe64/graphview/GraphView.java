...
abstract public class GraphView extends LinearLayout {
	...
	private class GraphViewContentView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			for (int i = 0; i < graphSeries.size(); i++) {
				drawSeries(canvas, graphSeries.get(i).getData(), graphwidth, graphheight, border,
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			if (!handled) {
				Log.d("GraphView", "on touch event scale not handled+"
						+ lastTouchEventX);
	...
	// GraphViewDataAdapterInterface values = graphSeries.get(idxSeries).values;
	// synchronized (values) {
	...
	// } else {
	// // viewport
	// List<GraphViewDataInterface> listData = new
	// ArrayList<GraphViewDataInterface>();
	...
	abstract protected void drawSeries(Canvas canvas,
			GraphViewDataAdapterInterface values, float graphwidth,
	...
	protected double getMaxX(boolean ignoreViewport) {
		...
		if (!ignoreViewport && viewportSize != 0) {
			...
			double highest = 0;
			if (graphSeries.size() > 0) {
				GraphViewDataAdapterInterface data = graphSeries.get(0)
						.getData();
				if (data.size() == 0) {
					highest = 0;
				} else {
					highest = data.getX(data.size() - 1);
				}
				for (int i = 1; i < graphSeries.size(); i++) {
					GraphViewDataAdapterInterface datas = graphSeries.get(i)
							.getData();
					if (datas.size() > 0) {
						highest = Math.max(highest,
								datas.getX(datas.size() - 1));
					}
				}
			}
			return highest;
	...
	protected double getMaxY() {
		...
		if (manualYAxis) {
			...
			for (int i = 0; i < graphSeries.size(); i++) {
				GraphViewDataAdapterInterface data = graphSeries.get(i)
						.getData();
				for (int ii = 0; ii < data.size(); ii++)
					if (data.getY(ii) > largest)
						largest = data.getY(ii);
			}
		}
		return largest;
	}
		...
		if (!ignoreViewport && viewportSize != 0) {
			...
			if (graphSeries.size() > 0) {
				...
				for (int i = 1; i < graphSeries.size(); i++) {
					...
					if (datas.size() > 0) {
						lowest = Math.min(lowest, datas.getX(0));
					}
				}
			}
			return lowest;
		...
		if (manualYAxis) {
			...
			for (int i = 0; i < graphSeries.size(); i++) {
				GraphViewDataAdapterInterface data = graphSeries.get(i)
						.getData();
				for (int ii = 0; ii < data.size(); ii++)
					if (data.getY(ii) < smallest)
						smallest = data.getY(ii);
			}
		}
		return smallest;
	}
