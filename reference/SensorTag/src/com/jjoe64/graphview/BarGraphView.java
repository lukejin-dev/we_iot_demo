...
public class BarGraphView extends GraphView {
	...
	@Override
	public void drawSeries(Canvas canvas, GraphViewDataAdapterInterface values, float graphwidth, float graphheight,
		...
		for (int i = 0; i < values.size(); i++) {
			float valY = (float) (values.getY(i) - minY);
			float ratY = (float) (valY / diffY);
			float y = graphheight * ratY;

			// hook for value dependent color
			if (style.getValueDependentColor() != null) {
				paint.setColor(style.getValueDependentColor().get(values.getX(i),values.getY(i)));
			}
